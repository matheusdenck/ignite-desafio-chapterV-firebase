...
class ToDoRemoteDataSource implements ToDoDataSourceInterface {
  ...
  @override
  Stream<List<ToDoItem>?> fetchToDoListStream() => toDoCollection.snapshots().map<List<ToDoItem>>(
        (event) => event.docs
            .map(
              (doc) => ToDoItem(
                id: doc.id,
  ...
  @override
  Future<void> onToggleToDoItemState(String uid, bool state) async {
    toDoCollection.doc(uid).update(
      <String, Object?>{'isDone': state},
    );
  }
}
